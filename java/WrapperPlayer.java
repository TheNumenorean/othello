...
/**
 * A wrapper class for the CS 2 Othello tournament. This should enable students
 * to use a non-Java Othello player by handing the moves back and forth using
 * stdin/stdout.
 */
public class WrapperPlayer implements OthelloPlayer {
	 ...
	 * 
	 * @param opponentsMove
	 *            the last move made by the other player. If it is null, then
	 *            that player passed, of this player is the first player to make
	 *            a move.
	 * 
	 * @param millisLeft
	 *            the number of milliseconds that this player has remaining in
	 *            the entire game, before going overtime and being disqualified.
	 * 
	 * @return Move this player's move. May be null only if there are no legal
	 *         moves to make.
	 */
	public Move doMove(Move opponentsMove, long millisLeft) {
		...
		try {
			if (opponentsMove == null) {
				...
				bw.write(opponentsMove.getX() + " " + opponentsMove.getY() + " " + millisLeft + "\n");
			}
			bw.flush();
	...
	public void init(OthelloSide side) {
		try {
			...
			if (name.endsWith(".jar")) {
				cmd = "java -jar " + name + " " + side;
			} else {
				cmd = "ulimit -m " + MAX_MEMORY_KB + " -v " + MAX_MEMORY_KB + ";";
				cmd += "./" + name + " " + side;
			}
